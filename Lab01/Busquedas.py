...
class Ventana(tk.Frame):
    ...
    def create_widgets(self):
        v = tk.IntVar()
        menu_msg = "" \
                   "Lab 01: Búsquedas ciegas y Heurísticas"
        self.menu_tktext = Label(root, text=menu_msg)
        self.menu_tktext.pack()

        self.boton1 = Button(root, text="Búsqueda ciega profundidad", command=lambda: flujo_principal(0,entry.get(),entry2.get(),entry3.get(),entry4.get(),entry5.get()))
...
class Graph():
    ...
    def A_AST(self, StartX, StartY, EndX, EndY):
        #Hallar nodo en si
        INICIO=0
        FIN=0
    ...
    def hill_climbing(self, n_nodos, nodo_ini, nodo_objetivo):
        ...
        print(((self.G.nodes[0]['coords'][1]-self.G.nodes[nodo_objetivo]['coords'][1])**2)+((self.G.nodes[nodo_objetivo]['coords'][0]-self.G.nodes[0]['coords'][0])**2))
    ...
    def PROF(self,StartX, StartY, EndX, EndY):
        INICIO=0
        FIN=0
        for i in self.coordinates:
            if(self.coordinates[i][0]==EndX and self.coordinates[i][1]==EndY):
    ...
    if(valor==1):
        ...
        for i in nd_deleted:
            if nd_deleted[i][0]==int(sx) and nd_deleted[i][1]==int(sy):
                ...
                break
        if se_ejecuta:
            grafo.A_AST(int(sx),int(sy),ax,ay)
            grafo.print_grafo()
    elif(valor==0):
        grafo=Graph(int(n),int(n))
        nd_deleted=grafo.eliminar_nodos()
        ...
        for i in nd_deleted:
            if nd_deleted[i][0]==int(sx) and nd_deleted[i][1]==int(sy):
                window = Tk()
                window.title("Error")
                lbl=Label(window,text="El nodo inicial fue eliminado por favor escoja otro :( ")
                lbl.grid(column=0,row=0)
                window.mainloop()
                se_ejecuta=False
                break
        if se_ejecuta:
            grafo.PROF(int(sx),int(sy),ax,ay)
            grafo.print_grafo()
    elif(valor==2):
        grafo=Graph(int(n),int(n))
        nd_deleted=grafo.eliminar_nodos()
